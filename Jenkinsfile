...
pipeline {
    environment {
        ...
        APP_EXPOSED_PORT = "80"
        IMAGE_TAG = "latest"
        STAGING = "cathalina-staging"
        PRODUCTION = "cathalina-prod"
        DOCKERHUB_ID = "CathalinaRanaivoarison"
        DOCKERHUB_PASSWORD = credentials('dockerhub_password')
        APP_NAME = "cathalina"
        STG_API_ENDPOINT = "ip10-0-21-4-cpisfdjj2epg008vt5k0-1993.direct.docker.labs.eazytraining.fr"
  ...
  post {
       success {
         slackSend (color: '#00FF00', message: "Cathalina - SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL}) - PROD URL => http://${PROD_APP_ENDPOINT} , STAGING URL => http://${STG_APP_ENDPOINT}")
         }
      failure {
            slackSend (color: '#FF0000', message: "Cathalina - FAILED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
          }   
    }  
}
